...
async def add_todo(db: db_depedency, insert_todo: TodoRequest):
    todo_model = models.Todos(**insert_todo.model_dump())
...
async def update_todo(
    ...
    # 1) ensure it exists
    exists = db.query(models.Todos).filter(models.Todos.id == todo_id).first()
    if not exists:
        raise HTTPException(status_code=404, detail="To Do not found")
    ...
    db.query(models.Todos).filter(models.Todos.id == todo_id).update(data)
    db.commit()
